...
### 电子邮件4——发送比特币时所涉及的攻击和IP地址

> **来自：“Satoshi Nakamoto”satoshi@vistomail.com**
>
> **致：dtrammell@dustintrammell.com**
>
> **日期：2009年1月16日星期五03:46:30 +0800**
>
> **主题：回复：一些想法......**
> ————————————————————————————————————————————————————
> **我将攻击分为两类：**
...
> **1）攻击只能由实际在通信链中的某人完成**
>
> **2）互联网上的任何人都可以从任何地方进行攻击**
>
> **类型1暴露您本地LAN上的人或公司中的人员，中间ISP的管理员以及收件人侧的LAN。类型2暴露给十亿人，他们可以自我选择成为攻击者，并在他们开发一种攻击多个受害者的技术时获得规模经济。**
>
> **通过IP发送请求一个新的公钥，所以是的，它很容易受到1型中间人的攻击。如果这是一个问题，发送到比特币地址没有这个漏洞，虽然有一个小的隐私权衡。我有一种感觉，大多数时候人们会从非SSL网站和未签名的明文电子邮件中获取比特币地址，这些电子邮件已经通过DNS中毒容易受到类型1和类型2的攻击。**
>
> **一种解决方案是在发送时使用IP和比特币地址（可能是1.2.3.4-1Kn8iojk...），其中收件人使用比特币地址的公钥来签署新公钥以证明您正在发送你认为自己是谁如果系统开始用于实际业务目的，我肯定会实现它。另一种解决方案是使用SSL。**
>
> **现在，很明显，如果您发送到IP，您没有提供有关收件人的任何其他识别信息，因此您盲目地发送给回答该IP的任何人。**
>
> **稍后的另一个功能是加密钱包的选项。**
> ————————————————————————————————————————————————————
> 如果我理解这是如何正确完成的，你只需将交易计算到区块链中，让目标接收者“发现”它，对吗？
> ————————————————————————————————————————————————————
> **那是对的**
> -———————————————————————————————————————————————————
> 另一种方法是允许网络节点提供解析服务，他们在那里询问一个比特币的网络地址，如果该节点在线，一旦网络就该地址达成共识，发送BitCoin应用程序就会直接连接到那里。
> ————————————————————————————————————————————————————
> **只需要比特币地址并在幕后制作IP就好了。可能存在隐私或拒绝服务问题。当然，在实施另一种发送方法之前，现在有足够的时间来充分考虑设计并确保它是最好的方式。**
>
> **聪**
...
> **致：dtrammell@dustintrammell.com**
>
> **日期：2009年1月17日星期六02:32:48 +0800**
>
> **主题：回复：一些想法......**
> ————————————————————————————————————————————————————
> &emsp; &emsp; 在这个主题上想到的一件事是，如果您遇到系统故障，BitCoin可能会丢失。应用程序似乎没有在它运行的目录中存储任何数据，所以我假设它存储在注册表和其他地方（尚未解析ProcessExplorer尚未检查我自己），所以它可能是一个好主意让应用程序能够将恢复所需的所有内容导出到可以从系统备份的文件中。
> ————————————————————————————————————————————————————
> **这些文件位于“％appdata％\Bitcoin”中，这是要备份的目录。数据存储在交易数据库DBM中，因此如果发生崩溃或电源故障，它应该是安全的。**
>
> **％appdata％是每用户访问权限。大多数像Firefox这样的新程序都会在那里存储他们的设置文件，尽管微软在每次Windows版本中都会改变目录名称并且空间不足而且它会在屏幕上运行很长时间。**
> ————————————————————————————————————————————————————
> 我今天注意到的另一件事是，如果你关闭应用程序，它似乎并没有干净地关闭它的网络套接字（TCP RST开始飞行）。可能是低优先级待办事项列表的项目（：
> ————————————————————————————————————————————————————
> **刚刚为下一个版本添加了代码。**
>
> **聪**
...
> **致：dtrammell@dustintrammell.com**
>
> **日期：2009年1月19日星期一00:54:32 0800**
>
> **主题：回复：比特币转账**
>
> **这应该是你在家里收到的比特币地址。没有办法让它知道它来自谁，所以它能做的最好的事情是告诉你收到了哪些地址。**
>
> **您可以创建多个地址并为每个人提供不同的地址，并标记它们以帮助确定谁发送给您。**
>
> **它除了你告诉它之外什么都不知道。在那里打印的名称是地址簿中与该地址相关的内容，可以是地址簿按钮或比特币地址右侧的“更改...”按钮。**
> ————————————————————————————————————————————————————
> &emsp; &emsp; 嘿，中本聪
>
> &emsp; &emsp; 在第一次转账25.00之后，你没有再向我发送100.00
>
